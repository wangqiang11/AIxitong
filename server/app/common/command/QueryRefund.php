...
class QueryRefund extends Command
{
        ...
        try {
            ...
            foreach ($refundRecords as $record) {
                ...
                if ($record['order_type'] == 'recharge') {
                    $order = (new RechargeOrder())->whereIn('id', $record['order_id'])->findOrEmpty()->toArray();
                }
     ...
     * @date 2023/3/1 15:54
     */
    public function checkReFundStatus($refundData): bool
    {
        $result = match ($refundData['pay_way']) {
            PayEnum::WECHAT_PAY => self::checkWechatRefund($refundData['terminal'], $refundData['log_sn']),
            PayEnum::ALI_PAY    => self::checkAliRefund($refundData['order_sn'], $refundData['log_sn']),
            default => null,
        };
     ...
     * @date 2023/3/1 15:47
     */
    public function checkWechatRefund($orderTerminal, $refundLogSn): mixed
    {
        // 根据商户退款单号查询退款
     ...
     */
    public function checkAliRefund($order_sn,$refundLogSn): array|string
    {
