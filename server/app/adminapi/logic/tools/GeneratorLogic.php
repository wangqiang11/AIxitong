...
class GeneratorLogic extends BaseLogic
{
    ...
    {
        $modelGenerateTable = new GenerateTable();
        $modelGenerateTable->startTrans();
        try {
            ...
            $modelGenerateTable->commit();
            return true;
        } catch (Exception $e) {
            $modelGenerateTable->rollback();
    ...
    public static function deleteTable($params): bool
    {
        ...
        $modelGenerateTable->startTrans();
        try {
            (new GenerateTable())->whereIn('id', $params['id'])->delete();
            (new GenerateColumn())->whereIn('table_id', $params['id'])->delete();
            $modelGenerateTable->commit();
            return true;
        } catch (Exception $e) {
            $modelGenerateTable->rollback();
    ...
    public static function syncColumn($params): bool
    {
        ...
        try {
            // table 信息
            $table = (new GenerateTable())->findOrEmpty($params['id']);
            // 删除旧字段
            (new GenerateColumn())->whereIn('table_id', $table['id'])->delete();
            // 获取当前数据表字段信息
     ...
     * @date 2022/6/24 9:43
     */
    public static function generate($params): array|bool
    {
        try {
            // 获取数据表信息
            $tables = (new GenerateTable())
                ->with(['table_column'])
     ...
     */
    public static function preview($params): bool
    {
        try {
            // 获取数据表信息
