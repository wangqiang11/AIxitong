<template>
	<view
		class="u-switch"
		:class="[valueCom == activeValue ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']"
		@tap="onClick"
		:style="[switchStyle]"
	>
		<view
...
export default {
	...
	computed: {
		...
		switchStyle() {
			...
			style.backgroundColor = this.valueCom ==  this.activeValue ? this.activeColor : this.inactiveColor;
			return style;
		},
		loadingColor() {
			return this.valueCom ==  this.activeValue ? this.activeColor : null;
		}
	},
	methods: {
		onClick() {
			if (!this.disabled && !this.loading) {
				...
				this.$emit("input", this.valueCom !==  this.activeValue ? this.activeValue : this.inactiveValue);
				this.$emit("update:modelValue", this.valueCom !==  this.activeValue ? this.activeValue : this.inactiveValue);
				// 放到下一个生命周期，因为双向绑定的value修改父组件状态需要时间，且是异步的
				this.$nextTick(() => {
					this.$emit("change", this.valueCom !==  this.activeValue ? this.activeValue : this.inactiveValue);
				});
			}
		}
	}
