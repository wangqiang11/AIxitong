<template>
    <div>
        ...
        >
            ...
            <el-card class="!border-none mt-4" shadow="never" v-if="formData.sms_notice.is_show">
            ...
            <el-card class="!border-none mt-4" shadow="never" v-if="formData.email_notice.is_show">
                <div class="font-medium mb-7">邮箱通知</div>
                <el-form-item label="开启状态" prop="email_notice.status" required>
                    <el-radio-group v-model="formData.email_notice.status">
                        <el-radio label="0">关闭</el-radio>
                        <el-radio label="1">开启</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="邮箱标题" prop="email_notice.title" required>
                    <div class="w-80">
                        <el-input
                            v-model="formData.email_notice.title"
                            placeholder="请输入模板ID"
                        />
                    </div>
                </el-form-item>
                <el-form-item label="短信内容" prop="email_notice.content" required>
                    <div class="flex-1">
                        <div class="w-full max-w-[320px]">
                            <el-input
                                type="textarea"
                                :autosize="{ minRows: 6, maxRows: 6 }"
                                v-model="formData.email_notice.content"
                            />
                        </div>
                        <div class="form-tips">
                            <div v-for="(item, index) in formData.email_notice.tips" :key="index">
                                {{ item }}
                            </div>
                        </div>
                    </div>
