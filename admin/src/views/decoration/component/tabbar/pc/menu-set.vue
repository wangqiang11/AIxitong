                ...
                <del-wrap @close="handleDelete(index)" class="max-w-[400px]">
                    <div class="bg-fill-light w-full p-4 mt-4">
                        ...
                        >
                            ...
                            >
                                <template #upload>
                                    <div class="upload-btn w-[80px] h-[80px]">
                                        <icon name="el-icon-Plus" :size="16" />
                                        <span v-if="type !== 'menu'" class="text-xs leading-5"> 未选中 </span>
                                    </div>
                                </template>
                            </material-picker>
                            <material-picker
                                v-model="element.selected"
                                v-if="element.selected !== undefined  && type !== 'menu'"
                                upload-class="bg-body"
                                exclude-domain
                                size="80px"
                            >
                        ...
                        <el-form-item label="导航名称" v-if="element.name !== undefined">
                            <div class="flex flex-1">
                                <div class="flex-1">
                                    <el-input class="w-full" v-model="element.name" placeholder="请输入名称" />
                                </div>
                                <el-checkbox class="ml-2 !h-[32px]" v-if="type === 'menu'" v-model="element.showName" size="large">
                                    {{ element.showName ? '显示' : '不显示' }}
                                </el-checkbox>
                            </div>
                        </el-form-item>
