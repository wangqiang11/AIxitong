<template>
    <div>
        ...
        <el-card class="!border-none mt-4" shadow="never">
            <el-button type="primary" v-perms="['digital.decalsCategory/add']" @click="openPop"
                >+ 新增分类</el-button
            >
            <el-table class="mt-2" size="large" v-loading="pager.loading" :data="pager.lists">
                ...
                <el-table-column label="关联贴纸数" prop="decals_count" min-width="100" />
                <el-table-column label="状态" min-width="100">
                    <template #default="{ row }">
                        <el-switch
                            v-perms="['digital.decalsCategory/status']"
                ...
                <el-table-column label="操作" width="120" fixed="right">
                    <template #default="{ row }">
                        <div>
                            <el-button
                                type="primary"
                                v-perms="['digital.decalsCategory/edit']"
                                link
                                @click="openEdit(row)"
                                >编辑</el-button
                            >
                            <el-button
                                type="danger"
                                v-perms="['digital.decalsCategory/del']"
                                @click="del(row.id)"
                                link
                                >删除</el-button
                            >
...
const { pager, getLists, resetPage, resetParams } = usePaging({
    fetchFun: getStickerCategoryList,
