 ...
 * @description 生成组题
 */
export const generateTheme = (options: Record<string, string>, isDark = false) => {
    ...
    const theme = Object.keys(varsMap).reduce((prev, key) => {
...
export const setTheme = (options: Record<string, string>, isDark = false) => {
    let theme = generateTheme(options, isDark)
    theme = `:root{${theme}}`
